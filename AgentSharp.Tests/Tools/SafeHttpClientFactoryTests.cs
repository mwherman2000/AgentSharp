...
public class SafeHttpClientFactoryTests
{
    [Theory]
    ...
    {
        Assert.True(SafeHttpClientFactory.IsBlockedAddress(IPAddress.Parse(ip)));
    }
    ...
    {
        Assert.False(SafeHttpClientFactory.IsBlockedAddress(IPAddress.Parse(ip)));
    }
}
